...
public class GraphHopper implements GraphHopperAPI
{
    ...
    protected List<Path> getPaths( GHRequest request, GHResponse rsp )
    {
        ...
        boolean uniquePaths = request.getHints().getBool("unique_paths", false);

        logger.info("Unique Paths are: " + uniquePaths + " ---" + request);

        AlreadyVisitedEdges alreadyVisitedEdges = null;
        if (uniquePaths)
            alreadyVisitedEdges = new AlreadyVisitedEdges();
        ...
        for (int placeIndex = 1; placeIndex < points.size(); placeIndex++)
        {
            ...
            if (uniquePaths && algo instanceof AbstractRoutingAlgorithm)
            {
